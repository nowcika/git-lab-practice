...
`reflog` 시나리오는 `cherry-pick -x`가 남긴 출처 줄의 SHA를 읽어, 그 커밋이 실제로 존재하고 브랜치의 조상이 아니며(정말 잃어버렸는지) 복구본과 patch-id가 같은지 확인합니다. 잃어버린 커밋은 push되지 않으므로 다른 PC에서 clone한 저장소에서는 판정을 보류합니다.
