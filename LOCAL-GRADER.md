...
이 프로젝트 폴더에서 다음 명령을 실행하세요. `--repo`에는 **학습자가 실제로 작업한 저장소 폴더**를 넣습니다.
...
`--username`을 생략하면 온라인에서는 인증 계정을 사용합니다. `--github owner/repo`로 대상 저장소를 지정할 수 있지만 origin이 존재하면 같은 저장소여야 합니다. HTTPS 및 SSH GitHub origin을 지원합니다. 오프라인 기본 브랜치는 `main`이며 `--branch master` 등으로 변경할 수 있습니다. 온라인은 GitHub의 기본 브랜치를 사용합니다.
...
점수는 `pass` 항목만 합산합니다. `unknown`이 있으면 `complete: false`이므로 확정 점수로 취급하면 안 됩니다. 종료 코드는 전체 통과 `0`, 실패 `1`, 확인 불가 또는 실행 오류 `2`입니다. JSON에는 검사 시각, 모드, HEAD, 항목별 배점, 로컬/원격 SHA, 파일 존재 여부와 저장소 상태가 들어갑니다. 기존 보고서는 덮어쓰지 않으므로 재실행할 때 새 파일명을 지정하세요.
...
이 검증기는 현재 웹페이지 채점 기준을 기반으로 하되 소유자·push·revert 검사를 강화했습니다. 공식 정답 `nowcika/git-scenario-solution`은 웹과 동일하게 원본 Fork 조건의 예외이며, 온라인 검증은 해당 소유 계정 인증이 필요합니다.
