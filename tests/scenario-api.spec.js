...
async function json(r,u){const x=await githubGet(r,u);expect(x.ok(),`${u}: ${x.status()}`).toBeTruthy();return x.json()}
...
async function missing(r,p,ref){return (await githubGet(r,`${api}/contents/${p}?ref=${encodeURIComponent(ref)}`)).status()===404}
async function commits(r,ref){return json(r,`${api}/commits?sha=${encodeURIComponent(ref)}&per_page=30`)}
test('배포 사이트와 20개 시나리오',async({request})=>{const x=await request.get('https://nowcika.github.io/git-lab-practice/');expect(x.ok()).toBeTruthy();expect(await x.text()).toContain('20개 평가 항목 · 325점');const s=await(await request.get('https://nowcika.github.io/git-lab-practice/scenarios.js')).text();expect(s).toContain('git-scenario-solution');expect((s.match(/\{ id:'/g)||[]).length).toBe(20)});
test('원본 문제 브랜치',async({request})=>{for(const n of ['upstream-update','conflict-left','conflict-right','revert','rebase-topic','rebase-base','rebase-flow-base','rebase-flow-topic','rebase-conflict-base','rebase-conflict-topic','reset','reflog-base','amend','cherry-source','diff-base','diff-target','show-source','patch-source','blame','branch-workflow','actions-release','pages'])expect((await githubGet(request,`${source}/branches/${encodeURIComponent(`scenario/${n}`)}`)).ok(),n).toBeTruthy()});
...
test('완료 tag',async({request})=>expect((await githubGet(request,`${api}/git/ref/tags/${encodeURIComponent('solution-v1.0.0')}`)).ok()).toBeTruthy());
