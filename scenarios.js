...
async function gradeScenarios() {
  ...
  const tasks=[
    ...
    async()=>{ const page=await scenarioContent(base,'docs/index.html','solution/pages'); const site=await api(`${base}/pages`); const source=!site.missing&&site.source; return page?.includes('PAGES-LIVE-2026')&&source?.branch==='solution/pages'&&source?.path==='/docs'&&site.status==='built'?passed(`Pages 서비스 확인: ${site.html_url}`):failed('index.html 문구와 Pages source(solution/pages, /docs), built 상태를 확인하세요.'); },
