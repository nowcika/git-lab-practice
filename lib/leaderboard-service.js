...
// 이 프로젝트가 운영하는 저장소는 참가 기록이 아니므로 순위에 등록하지 않습니다.
// 순위표 자신(PLATFORM), 문제·외부 자료·정답·예제 저장소가 모두 해당합니다.
const BLOCKED_REPOSITORIES = new Set([
  PLATFORM, 'nowcika/git-scenario-lab', 'nowcika/git-scenario-library',
...
function parseIssue(issue) {
  ...
  if (BLOCKED_REPOSITORIES.has(repository.toLowerCase())) throw new Error('이 프로젝트의 공식 운영 저장소(순위표·문제·정답·예제)는 순위에 등록하지 않습니다.');
...
function parseDeletion(issue) {
  if (!issue || issue.pull_request || issue.state !== 'open' || !issue.title?.startsWith('[Leaderboard]')
    || !Number.isSafeInteger(issue.number) || !Number.isSafeInteger(issue.user?.id)
    || issue.user.id <= 0 || !Core.validLogin(issue.user?.login) || issue.user.type !== 'User') throw new Error('삭제 요청 작성자를 확인할 수 없습니다.');
  const body = String(issue.body || '');
  const fields = {};
  for (const match of body.matchAll(/^### (Action|Course)\r?\n+([^\r\n]+)(?:\r?\n|$)/gm)) {
    if (fields[match[1]]) throw new Error('중복된 삭제 항목입니다.');
    fields[match[1]] = match[2].trim();
  }
  if (fields.Action !== 'delete' || !Core.COURSES[fields.Course]) throw new Error('삭제 요청 양식을 확인하세요.');
  return { userId: issue.user.id, login: issue.user.login, course: fields.Course, issueNumber: issue.number, bodyHash: digest(body) };
}
const isDeletion = issue => /^### Action\r?\n+delete(?:\r?\n|$)/m.test(String(issue?.body || ''));
async function deleteRecord(client, report) {
  const request = report.deletion;
  if (!request || !Number.isSafeInteger(request.issueNumber) || request.issueNumber <= 0) throw new Error('삭제 요청 형식 오류');
  for (let attempt = 0; attempt < 5; attempt++) {
    const current = parseDeletion(await client.request(`/repos/${PLATFORM}/issues/${request.issueNumber}`));
    if (current.bodyHash !== request.bodyHash || current.userId !== request.userId || current.course !== request.course) throw new Error('삭제 요청이 변경됐습니다.');
    let file;
    try { file = await client.request(`/repos/${PLATFORM}/contents/${FILE}?ref=${BRANCH}`); }
    catch (error) { if (error.status === 404) return { changed: false }; throw error; }
    if (file.encoding !== 'base64' || !file.content) throw new Error('순위 파일을 읽지 못했습니다.');
    const board = Core.validateBoard(JSON.parse(Buffer.from(file.content, 'base64').toString('utf8')));
    const entries = board.entries.filter(e => !(e.userId === current.userId && e.course === current.course));
    if (entries.length === board.entries.length) return { changed: false };
    const next = { ...board, entries, updatedAt: new Date().toISOString() };
    try {
      await client.request(`/repos/${PLATFORM}/contents/${FILE}`, 'PUT', { branch: BRANCH, sha: file.sha,
        message: `Remove own ${current.course} leaderboard record`, content: Buffer.from(`${JSON.stringify(next, null, 2)}\n`).toString('base64') });
      return { changed: true };
...
function resultEntry(submission, checks, runId, checkedAt = new Date().toISOString()) {
  ...
  const entry = {
    ...
    assessment: { checkedAt, runId: Number(runId), checks: checks.map(({ name, points, state, detail }) => ({ name, points, state, detail })) },
  };
...
module.exports = { BLOCKED_REPOSITORIES, parseDeletion, isDeletion, deleteRecord, PLATFORM, BRANCH, FILE, digest, parseIssue, GitHubClient, verifyOwner, resultEntry, publish };
