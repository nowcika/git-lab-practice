...
class Repository {
  ...
  upstreamRef(name) {
    const refs = (this.git(['for-each-ref', '--format=%(refname)', `refs/remotes/*/${name}`], true) || '').split('\n').filter(Boolean);
    return refs.length ? this.sha(refs[0]) : null;
  }
...
const scenarioSpecs = [
  ...
  ['pullrebase', 'push 거부 후 rebase', 15, 'pull-rebase', (c, e) => {
    ...
    if (baseTip && c.repo.git(['merge-base', '--is-ancestor', baseTip, c.sha], true) === null) {
...
async function grade(options, services = {}) {
  ...
  if (course === 'basic') {
    ...
    await check('branch', 'practice/feature 브랜치와 변경', 15, async e => {
      ...
      if (!(e.ahead > 0)) { e.reason = 'practice/feature가 기본 브랜치와 같습니다. 브랜치에서 파일을 수정해 커밋하세요.'; return false; }
