...
class Repository {
  ...
  // upstream 등 원격 이름은 학습자마다 다를 수 있어 경로로 찾습니다.
  upstreamRef(name) {
    for (const remote of (this.git(['remote']) || '').split('\n').filter(Boolean)) {
      const source = parseGitHub(this.git(['remote', 'get-url', remote]) || '');
      if (source?.toLowerCase() !== 'nowcika/git-scenario-lab') continue;
      const sha = this.sha(`refs/remotes/${remote}/${name}`);
      if (sha) return sha;
    }
    return null;
  }
  ...
  // 웹 편집기·API로 만든 커밋은 committer가 GitHub <noreply@github.com>입니다.
  // clone한 저장소에도 그대로 남으므로 오프라인에서도 구분할 수 있습니다.
  localCommits(sha) {
    if (this.shallow) throw new Unavailable('shallow clone: 전체 이력이 없어 판정할 수 없습니다.');
    return this.git(['log', '--format=%H%x00%ce%x00%cn%x00%x1e', sha, '--'])
      .split('\x1e').map(row => row.trim()).filter(Boolean)
      .map(row => { const [id, email, name] = row.split('\0'); return { sha: id, email, name }; })
      .filter(commit => !(commit.email === 'noreply@github.com' && commit.name === 'GitHub'));
  }
  aheadCount(base, head) {
    const value = this.git(['rev-list', '--count', `${base}..${head}`], true);
  ...
  changeId(sha) {
    // 커밋이 부모에 대해 만든 변경의 고유 id입니다. 같은 변경이면 SHA가 달라도 같습니다.
    const parent = this.git(['rev-parse', '--verify', `${sha}^1`], true);
    if (!parent) return null;
    const diff = this.git(['diff', '--no-ext-diff', '--no-textconv', parent, sha, '--']);
    return diff ? this.patchId(diff) : null;
  }
  revertEvidence(sha) {
...
const scenarioSpecs = [
  ...
  ['pullrebase', 'push 거부 후 rebase', 15, 'pull-rebase', (c, e) => {
    if (!includes(c.file('service/api.md'), 'FLOW-BASE-API-2026')) { e.reason = 'service/api.md(동료 변경)가 없습니다.'; return false; }
    if (!includes(c.file('service/ui.md'), 'FLOW-TOPIC-UI-2026')) { e.reason = 'service/ui.md(내 변경)가 없습니다.'; return false; }
    if (!c.linear()) { e.reason = 'merge 커밋이 있습니다. git pull --rebase로 정리하세요.'; return false; }
    if (!/^feat: add ui service note/.test(c.history()[0].message)) { e.reason = '내 커밋이 맨 위에 있어야 합니다.'; return false; }
    const baseTip = c.repo.upstreamRef('scenario/rebase-flow-base');
    if (!baseTip) throw new Unavailable('원본 기준 브랜치를 찾을 수 없습니다. git fetch upstream 후 다시 검사하세요.');
    if (c.repo.git(['merge-base', '--is-ancestor', baseTip, c.sha], true) === null) {
      e.reason = '동료 커밋이 이력에 없습니다. 기준 브랜치 위로 rebase하세요.'; return false;
    }
    e.baseTip = baseTip; return true;
  }],
  ['rebaseconflict', 'rebase 충돌 해결', 20, 'rebase-conflict', (c, e) => {
    const target = c.file('config/deploy-target.md');
    if (target === null) { e.reason = 'config/deploy-target.md가 없습니다.'; return false; }
    if (/<{7}|={7}|>{7}/.test(target)) { e.reason = '충돌 표시가 그대로 남아 있습니다.'; return false; }
    const missing = ['배포 대상: production', '담당: 플랫폼팀', '승인 코드: REBASE-CONFLICT-2026'].filter(line => !target.includes(line));
    if (missing.length) { e.reason = `해결 결과에 빠진 줄: ${missing.join(' / ')}`; return false; }
    if (!c.linear()) { e.reason = 'merge 커밋이 있습니다. rebase로 해결하세요.'; return false; }
    const baseTip = c.repo.upstreamRef('scenario/rebase-conflict-base');
    if (!baseTip) throw new Unavailable('원본 기준 브랜치를 찾을 수 없습니다. git fetch upstream 후 다시 검사하세요.');
    if (c.repo.git(['merge-base', '--is-ancestor', baseTip, c.sha], true) === null) {
      e.reason = '기준 브랜치 커밋이 이력에 없습니다. 기준 위로 rebase하세요.'; return false;
    }
    e.baseTip = baseTip; return true;
  }],
  ...
  ['reflog', '복구 결과', 15, 'reflog', (c, e) => {
    if (!includes(c.file('recovered-note.txt'), 'REFLOG-RECOVERED-COMMIT')) {
      e.reason = 'recovered-note.txt에서 고유 문구를 찾지 못했습니다.'; return false;
    }
    const restored = c.history().find(h => /add recoverable note/.test(h.message));
    if (!restored) { e.reason = '"docs: add recoverable note" 커밋을 찾지 못했습니다.'; return false; }
    const origin = /\(cherry picked from commit ([0-9a-f]{7,40})\)/i.exec(restored.message)?.[1];
    if (!origin) { e.reason = '복구 커밋에 출처 줄이 없습니다. git cherry-pick -x <잃어버린-SHA> 로 되살리세요.'; return false; }
    if (restored.sha.startsWith(origin)) { e.reason = '출처 줄이 복구 커밋 자신을 가리킵니다.'; return false; }
    e.recovery = { restored: restored.sha, origin };
    // 잃어버린 커밋은 push되지 않으므로 다른 PC에서 clone한 저장소에는 없습니다.
    if (c.repo.objectType(origin) !== 'commit') {
      throw new Unavailable('잃어버린 원본 커밋이 이 저장소에 없습니다. 실제로 작업한 PC에서 검사하세요(reflog와 고아 커밋은 clone으로 옮겨지지 않습니다).');
    }
    // 정말 브랜치에서 떨어져 나간 커밋이어야 합니다. 조상으로 남아 있으면 잃어버린 적이 없습니다.
    if (c.repo.git(['merge-base', '--is-ancestor', origin, c.sha], true) !== null) {
      e.reason = '출처 커밋이 아직 브랜치 이력에 남아 있습니다. reset으로 잃어버린 커밋을 되살린 것이 아닙니다.'; return false;
    }
    // 같은 변경을 되살렸는지 patch-id로 대조합니다.
    const originChange = c.repo.changeId(origin);
    const restoredChange = c.repo.changeId(restored.sha);
    if (!originChange || originChange !== restoredChange) {
      e.reason = '되살린 커밋의 변경 내용이 출처 커밋과 다릅니다.'; return false;
    }
    e.recovery.patchId = originChange;
    return true;
  }],
...
async function grade(options, services = {}) {
  ...
  const reflogEntries = (repo.git(['reflog', '--all', '--format=%gs'], true) || '').split('\n').filter(Boolean);
  diagnostics.push({
    id: 'reflog', entries: reflogEntries.length,
    reset: reflogEntries.some(entry => entry.startsWith('reset:')),
    cherryPick: reflogEntries.some(entry => entry.startsWith('cherry-pick')),
  });
  ...
  if (course === 'basic') {
    ...
    await check('repository', '직접 만든 공개 저장소', 15, e => {
      e.repository = target; e.originMatches = !wrongOrigin;
      if (!trustedRepository(e)) return false;
      if (metadata.fork) { e.reason = 'Fork한 저장소는 인정하지 않습니다. 직접 새로 만든 저장소를 사용하세요.'; return false; }
      return true;
    });
    ...
    await check('commits', '로컬에서 push한 커밋 2개', 15, async e => {
      if (!await sync(defaultBranch, e)) return false;
      const head = repo.branch(defaultBranch);
      e.count = repo.history(head).length;
      e.distinctTrees = new Set(repo.git(['log', '--format=%T', head, '--']).split('\n')).size;
      if (e.count < 2) { e.reason = '기본 브랜치의 커밋이 2개 미만입니다.'; return false; }
      if (e.distinctTrees < 2) { e.reason = '커밋은 2개지만 파일 내용이 바뀌지 않았습니다.'; return false; }
      // 이 과정의 목적은 로컬 Git 사용입니다. 웹 편집기로만 만든 커밋은 인정하지 않습니다.
      e.localCommits = repo.localCommits(head).length;
      if (e.localCommits < 2) {
        e.reason = `GitHub 웹 편집기로 만든 커밋만 있습니다(로컬 커밋 ${e.localCommits}개). 터미널에서 commit 후 push하세요.`;
        return false;
      }
      return true;
    });
    await check('branch', 'practice/feature 브랜치와 변경', 15, async e => {
      if (!await sync('practice/feature', e)) return false;
      e.ahead = repo.aheadCount(defaultBranch, 'practice/feature');
      if (!(e.ahead > 0)) {
        requireOnline();
        const featureSha = repo.branch('practice/feature');
        const merged = await github.find(`/repos/${target}/pulls?state=closed`, p => p.merged_at && p.head?.sha === featureSha
          && p.head?.ref === 'practice/feature' && p.base?.ref === defaultBranch
          && p.head?.repo?.full_name?.toLowerCase() === target.toLowerCase()
          && p.base?.repo?.full_name?.toLowerCase() === target.toLowerCase()
          && p.user?.login?.toLowerCase() === username?.toLowerCase());
        if (!merged) { e.reason = '기본 브랜치보다 앞선 변경이나 현재 커밋에 해당하는 병합된 PR이 없습니다.'; return false; }
        const commits = await github.get(`/repos/${target}/pulls/${merged.number}/commits?per_page=100`);
        if (!Array.isArray(commits)) throw new Unavailable('병합된 PR의 커밋을 확인할 수 없습니다.');
        const local = new Set(repo.localCommits(featureSha).map(c => c.sha));
        if (!commits.some(c => local.has(c.sha))) {
          if (commits.length >= 100) throw new Unavailable('병합된 PR의 커밋 조회 범위를 초과했습니다.');
          e.reason = '병합된 PR에서 로컬 커밋 조건을 만족하는 변경을 찾지 못했습니다.'; return false;
        }
        e.mergedPr = merged.number; return true;
      }
      const webOnly = repo.localCommits('practice/feature').filter(c => repo.git(['merge-base', '--is-ancestor', c.sha, defaultBranch], true) === null);
      if (!webOnly.length) { e.reason = 'practice/feature의 새 커밋이 웹 편집기로 만들어졌습니다.'; return false; }
      return true;
    });
    await check('issue', '내가 만든 학습 계획 이슈', 10, async e => {
      if (!trustedRepository(e)) return false;
    ...
    await check('pr', '내가 만든 feature PR', 10, async e => {
      if (!trustedRepository(e)) return false;
      const pr = await github.find(`/repos/${target}/pulls?state=all`, p => p.head?.ref === 'practice/feature' && p.base?.ref === defaultBranch && p.head?.repo?.full_name?.toLowerCase() === target.toLowerCase() && p.base?.repo?.full_name?.toLowerCase() === target.toLowerCase() && p.user?.login?.toLowerCase() === username?.toLowerCase());
      e.number = pr?.number;
  ...
  return {
    ...
    score: results.reduce((sum, r) => sum + r.earned, 0), maxScore: course === 'basic' ? 100 : 325,
