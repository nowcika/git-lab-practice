...
async function gradePublic(submission, token, { client = new GitHubClient(token) } = {}) {
  ...
  try {
    ...
    if (submission.course === 'basic') {
      ...
      checks = await page.evaluate(() => lastReport?.results.slice(1) || null);
    } else {
      await page.evaluate(() => {
        const renderOriginal = renderScenarioResults;
        renderScenarioResults = (checks, score) => {
          window.publicChecks = checks.map((check, index) => ({ ...check, points: scenarioDefinitions[index].points, name: scenarioDefinitions[index].title }));
          renderOriginal(checks, score);
        };
      });
      ...
      checks = await page.evaluate(() => window.publicChecks || null);
    }
