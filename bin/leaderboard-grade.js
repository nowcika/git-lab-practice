...
const { PLATFORM, parseIssue, digest, GitHubClient, resultEntry } = require('../lib/leaderboard-service');
...
async function main() {
  ...
  if (process.env.GITHUB_EVENT_NAME === 'issues') {
    ...
    submission = parseIssue(event.issue);
    ...
    submission = parseIssue(await client.request(`/repos/${PLATFORM}/issues/${number}`));
    publishable = true;
  } else {
