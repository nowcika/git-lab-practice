...
페이지 하단 **공개 순위표**에 본인 결과를 등록할 수 있습니다. 별도 가입이나 토큰 없이 GitHub 계정만 사용합니다.

1. 채점 결과에서 **공개 순위에 등록하기**를 누르거나 순위표에서 과정과 저장소를 고릅니다.
2. 공개 동의에 체크하면 GitHub 제출 양식으로 이동합니다. 로그인한 계정이 곧 제출자 신원입니다.
3. GitHub Actions가 **서버에서 다시 채점**합니다. 제출자가 적은 점수나 브라우저 값은 쓰지 않습니다.
4. 몇 분 뒤 순위표에 사용자별 **최고 점수**가 반영됩니다. 동점은 공동 순위이며, 행의 **채점 상세**에서 항목별 통과 여부를 볼 수 있습니다.

- 점수: 초급 **90점**(설치 출력 10점은 서버가 증명할 수 없어 제외), 실전 **325점**. 개인 연습 채점은 기존대로 100점입니다.
- 등록 조건: 본인 계정 소유의 공개 저장소여야 합니다. 이 프로젝트가 운영하는 저장소(순위표·문제·정답·예제)는 등록되지 않습니다.
- 공개 범위: 사용자 이름, 저장소, 점수, 항목별 결과, 검사 시각, 제출 이슈와 Actions 기록이 공개됩니다.
- 등록은 **선택**입니다. 일반 채점만 실행하면 순위표에 올라가지 않습니다. 순위표 아래 **내 순위 기록 삭제**로 본인 기록만 언제든 뺄 수 있습니다.

운영 방법, 신뢰 범위와 한계, 유지보수 시 지켜야 할 계약은 [LEADERBOARD.md](LEADERBOARD.md)에 있습니다.
